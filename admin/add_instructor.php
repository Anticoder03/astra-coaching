...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $email = $_POST['email'] ?? '';
    $phone = $_POST['phone'] ?? '';
    $image_url = $_POST['image_url'] ?? 'https://via.placeholder.com/150';

    if (empty($error)) {
        $stmt = $conn->prepare("INSERT INTO instructors (name, specialization, experience_years, bio, email, phone, image_url) VALUES (?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssissss", $name, $specialization, $experience_years, $bio, $email, $phone, $image_url);

        if ($stmt->execute()) {
            header('Location: dashboard.php?message=Instructor added successfully');
            exit;
        } else {
...
<head>
    ...
    <title>Add Instructor - Astra Coaching</title>
            ...
            <div class="bg-white shadow-lg">
                <div class="container mx-auto px-6 py-3">
                    <div class="flex items-center justify-between">
                        <div>
                            <h1 class="text-2xl font-bold text-gray-800">Add New Instructor</h1>
                        </div>
